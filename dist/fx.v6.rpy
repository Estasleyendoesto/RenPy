...
init -1 python:
    ...
    class FX:
        ...
        def autocamera(self, *args, **kwargs):
            self.autocamfx = AutocamFX(*args, **kwargs)
        ...
        def layer(self, *args, **kwargs):
            if self.parallax is None:
                if self.camfx:         
                    ...
                    self.parallax = Parallaxfx(self.autocamfx)
                else:
                    raise Exception("camfx or autocamfx object doesn't exists")
        ...
        def alive(self, *args, **kwargs):
            if self.alivefx is None:
                ...
                if self.autocamfx:
                    self.alivefx.cam = self.autocamfx
                if self.parallax:
                    self.alivefx.parallax = self.parallax

            self.alivefx.add(*args, **kwargs)
            
        def currcamfx(self, cam):
            if isinstance(cam, CamFX):
                self.camfx = cam
            if isinstance(cam, AutocamFX):
                self.autocamfx = cam
